...
class ApiMappingsTest {
  ...
  void mapsEveryOperationalState() {
    ...
    assertThat(mapped)
        .containsExactlyInAnyOrder(io.github.tamawish.rwr.api.model.ManagedWorldState.values());
  }

  @Test
  void mapsEveryPhaseAndSafety() {
    assertThat(
            Arrays.stream(ResetPhase.values()).map(ApiMappings::phase).collect(Collectors.toSet()))
        .containsExactlyInAnyOrder(io.github.tamawish.rwr.api.model.ResetPhase.values());
    assertThat(
            Arrays.stream(FailureSafety.values())
                .map(ApiMappings::safety)
                .collect(Collectors.toSet()))
  ...
  void mapsEveryFailureWithoutGenericFallback() {
    Set<io.github.tamawish.rwr.api.model.ResetFailureType> mapped =
        Arrays.stream(ResetFailureType.values())
            .map(ApiMappings::failure)
            .collect(Collectors.toSet());
    assertThat(mapped)
