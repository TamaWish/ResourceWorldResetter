...
/** Adapts Multiverse-Core world operations to the provider-neutral core interface. */
public final class MultiverseApiGateway implements WorldProvider {
  ...
  MultiverseApiGateway(
      WorldManager worldManager, BlockSafety blockSafety, RegenerationOptionsMapper optionsMapper) {
  ...
  public DestinationResult resolveSafeDestination(String name) {
    if (worldManager.getWorld(name).isEmpty()) {
      return new DestinationResult.Unavailable(
          DestinationFailureReason.NOT_REGISTERED, "World is not registered in Multiverse-Core.");
    }
   ...
   * legacy name supplied by the Multiverse world itself. Multiverse 5.8 otherwise reuses the stored
   * namespaced key during regen; Spigot cannot construct a world from that key and only reports the
   * problem after the old world has already been deleted.
   */
  private RegenerationOutcome regenerateUsingMultiverseLegacyName(
    ...
    CreateWorldOptions create =
        CreateWorldOptions.worldName(createName)
            .biome(world.getBiome())
            .bonusChest(
                world.getBukkitWorld().map(WorldCompatibility::hasBonusChest).getOrElse(false))
    ...
    Attempt<LoadedMultiverseWorld, org.mvplugins.multiverse.core.world.reasons.CreateFailureReason>
        creation = worldManager.createWorld(create);
