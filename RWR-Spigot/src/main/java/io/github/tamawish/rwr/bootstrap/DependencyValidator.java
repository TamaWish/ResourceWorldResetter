...
/** Validates that the required Multiverse plugin is enabled and API-compatible. */
public final class DependencyValidator {
  ...
  /**
   * Validates the installed Multiverse plugin.
   *
  ...
  public ValidationResult validate() {
    ...
    try {
      ...
      return ValidationResult.failure(
          "Cannot parse the installed Multiverse-Core version: " + pluginVersion(multiverse));
    }
    ...
    if (installed.compareTo(MINIMUM_MULTIVERSE_VERSION) < 0
      ...
      return ValidationResult.failure(
          "Multiverse-Core "
              + MINIMUM_MULTIVERSE_VERSION
              + " through 5.x is required; found "
              + installed
              + '.');
    }
    if (!MultiverseCoreApi.isLoaded()) {
      return ValidationResult.failure(
          "Multiverse-Core is enabled but its typed API is not initialized.");
    }
  ...
  /**
   * Result of validating the Multiverse dependency.
   *
