...
class NextRunCalculatorTest {
  ...
  void calculatesDailyAndWeeklyRunsInConfiguredZone() {
    ...
    ZonedDateTime daily =
        calculator.nextRun(
            new ScheduleSettings(ScheduleType.DAILY, LocalTime.of(3, 0), null, 0, 0), zone, after);
    ZonedDateTime weekly =
        calculator.nextRun(
  ...
  void clampsMonthlyDayToShortMonthThenRestoresRequestedDay() {
    ...
    ZonedDateTime february =
        calculator.nextRun(schedule, zone, ZonedDateTime.of(2026, 1, 31, 3, 0, 0, 0, zone));
  ...
  void intervalUsesElapsedTimeAcrossDaylightSavingTransition() {
    ...
    ZonedDateTime result =
        calculator.nextRun(
            new ScheduleSettings(ScheduleType.INTERVAL, null, null, 0, 30), zone, after);

    assertThat(Duration.between(after.toInstant(), result.toInstant()))
