...
/** Validates cross-field configuration rules against the provider world catalog. */
public final class ConfigValidator {
  private static final Pattern WORLD_ID = Pattern.compile("[a-z0-9][a-z0-9_-]{0,63}");

  /**
   * Validates a complete settings snapshot without mutating it.
   *
   * @param settings candidate configuration
   * @param catalog current provider world catalog
   * @return every validation issue discovered, or an empty list when valid
   */
  public List<ConfigIssue> validate(PluginSettings settings, WorldCatalogView catalog) {
    ...
    } else if (isNamespacedWorldName(settings.defaultHubWorld())
        && !catalog.allowsNamespacedWorldNames()) {
      issues.add(
          new ConfigIssue("default-hub-world", "must be a plain Bukkit world name without ':'"));
    ...
    for (var entry : settings.worlds().entrySet()) {
      ...
      if (!entry.getKey().equals(world.id())) {
        issues.add(
            new ConfigIssue("worlds." + entry.getKey(), "map key must match the stable world ID"));
      }
      if (!WORLD_ID.matcher(world.id()).matches()) {
        issues.add(new ConfigIssue(path, "ID must match " + WORLD_ID.pattern()));
      }
      if (!multiverseNames.add(
          catalog.canonicalWorldName(world.multiverseWorld()).toLowerCase(Locale.ROOT))) {
        issues.add(
            new ConfigIssue(path + ".multiverse-world", "is configured under more than one ID"));
      }
      if (world.multiverseWorld().isBlank()) {
        issues.add(new ConfigIssue(path + ".multiverse-world", "must not be blank"));
      } else if (isNamespacedWorldName(world.multiverseWorld())
          && !catalog.allowsNamespacedWorldNames()) {
        issues.add(
            new ConfigIssue(
      ...
      if (world.managed() && isProtected(world.multiverseWorld(), settings, catalog)) {
        issues.add(
            new ConfigIssue(path + ".managed", "the default or hub world cannot be managed"));
      }
  ...
  private static void validateSchedule(
      String path, ScheduleSettings schedule, List<ConfigIssue> issues) {
    ...
    switch (schedule.type()) {
      ...
      default -> throw new AssertionError("Unhandled schedule type: " + schedule.type());
    }
  }
  ...
  private static void validateWarnings(
      String path, List<Integer> warnings, List<ConfigIssue> issues) {
    ...
    for (int warning : warnings) {
      ...
      if (warning > previous) {
        issues.add(
            new ConfigIssue(path + ".warning-minutes", "values must be in descending order"));
      }
      previous = warning;
    }
  }
  ...
  private static void validateEvacuation(
    ...
    if (evacuation.destination().isBlank()) {
      issues.add(
          new ConfigIssue(
              path + ".evacuation.destination", "is required when evacuation is enabled"));
    } else if (isNamespacedWorldName(evacuation.destination())
        && !catalog.allowsNamespacedWorldNames()) {
      issues.add(
          new ConfigIssue(
  ...
  private static void validateTeleport(
      TeleportSettings teleport, WorldCatalogView catalog, List<ConfigIssue> issues) {
    ...
    teleport
        ...
        .forEach(
            (worldName, destination) -> {
              ...
              } else if (isNamespacedWorldName(worldName)
                  && !catalog.allowsNamespacedWorldNames()) {
                issues.add(
                    new ConfigIssue(path, "must use a plain Multiverse world name without ':'"));
              }
              if (!names.add(catalog.canonicalWorldName(worldName).toLowerCase(Locale.ROOT))) {
                issues.add(
                    new ConfigIssue(path, "duplicates another teleport override ignoring case"));
              }
  ...
  static boolean isProtected(String worldName, PluginSettings settings, WorldCatalogView catalog) {
    return catalog.sameWorld(worldName, settings.defaultHubWorld())
        || catalog.sameWorld(worldName, catalog.defaultWorldName());
  }
