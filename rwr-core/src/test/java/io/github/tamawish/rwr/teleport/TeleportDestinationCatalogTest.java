...
class TeleportDestinationCatalogTest {
  ...
  void explicitOverridesRemainIndependentWhenDiscoveryIsDisabled() {
    ...
    TeleportSettings settings =
        settings(false, false, true, Map.of("resource", destination(true, "Mining World", null)));

    List<TeleportDestinationView> result =
        catalog(gateway, Set.of()).destinations(settings, value -> false);
  ...
  void discoveryUsesDefaultsAndOmitsDisabledDestinations() {
    ...
    TeleportSettings settings =
        settings(true, true, true, Map.of("resource", destination(false, "Hidden", null)));
  ...
  void customAndWildcardPermissionsControlLockedState() {
    ...
    TeleportSettings settings =
        settings(false, false, true, Map.of("resource", destination(true, "Resource", "rank.vip")));
    ...
    assertThat(
            catalog.destinations(settings, TeleportDestinationCatalog.WILDCARD_PERMISSION::equals))
        .singleElement()
        .extracting(TeleportDestinationView::state)
        .isEqualTo(TeleportDestinationState.AVAILABLE);
  }
  ...
  void unloadedAndResettingWorldsAreUnavailableWithoutImplicitLoading() {
    ...
    assertThat(result)
        .filteredOn(destination -> destination.worldName().equals("offline"))
        .singleElement()
        .extracting(TeleportDestinationView::state)
        .isEqualTo(TeleportDestinationState.UNAVAILABLE);
    assertThat(result)
        .filteredOn(destination -> destination.worldName().equals("resetting"))
        .singleElement()
        .extracting(TeleportDestinationView::state)
  ...
  @Test
  void namespacedOverrideControlsDiscoveredWorldWithoutDuplicateDestinations() {
    WorldSnapshot snapshot =
        new WorldSnapshot(
            ...
            true,
            "resource 0 64 0");
    FakeGateway gateway = gateway(snapshot);
    TeleportDestinationCatalog catalog = catalog(gateway, Set.of());
    TeleportSettings restricted =
        settings(
            true,
            true,
            true,
            Map.of("worlds:resource", new TeleportDestinationSettings(true, "resource.access")));

    assertThat(catalog.destinations(restricted, permission -> false))
        .singleElement()
        .satisfies(
            destination -> {
              assertThat(destination.worldName()).isEqualTo("resource");
              assertThat(destination.state()).isEqualTo(TeleportDestinationState.LOCKED);
            });
    TeleportSettings disabled =
        settings(
            true,
            true,
            true,
            Map.of("worlds:resource", new TeleportDestinationSettings(false, null)));
    assertThat(catalog.destinations(disabled, permission -> true)).isEmpty();
  }

  @Test
  void menuEnumeratesWorldsOnceWithoutResolvingKnownNamesIndividually() {
    List<WorldSnapshot> worlds = new ArrayList<>();
    Map<String, TeleportDestinationSettings> overrides = new java.util.LinkedHashMap<>();
    for (int index = 0; index < 100; index++) {
      String name = "resource_" + index;
      worlds.add(world(name, true));
      overrides.put(name, new TeleportDestinationSettings(true, null));
    }
    FakeGateway gateway = gateway(worlds.toArray(WorldSnapshot[]::new));

    TeleportPage page =
        catalog(gateway, Set.of()).page(settings(true, true, true, overrides), value -> true, 0);

    assertThat(page.destinations()).hasSize(45);
    assertThat(page.pageCount()).isEqualTo(3);
    assertThat(gateway.catalogCalls).isEqualTo(1);
    assertThat(gateway.worldLookupCalls).isZero();
  }

  @Test
  void clickRevalidatesPermissionsResetStateAndWorldAvailabilityAfterRendering() {
    FakeGateway gateway = gateway(world("resource", true));
    Set<String> resetting = new HashSet<>();
    TeleportDestinationCatalog catalog = catalog(gateway, resetting);
    TeleportSettings settings =
        settings(
            false,
            false,
            true,
            Map.of("resource", new TeleportDestinationSettings(true, "resource.access")));
    assertThat(catalog.page(settings, value -> true, 0).destinations().getFirst().state())
        .isEqualTo(TeleportDestinationState.AVAILABLE);

    assertThat(catalog.destination(settings, value -> false, "resource").orElseThrow().state())
        .isEqualTo(TeleportDestinationState.LOCKED);
    resetting.add("resource");
    assertThat(catalog.destination(settings, value -> true, "resource").orElseThrow().state())
        .isEqualTo(TeleportDestinationState.RESETTING);
    gateway.worlds = List.of(world("resource", false));
    assertThat(catalog.destination(settings, value -> true, "resource").orElseThrow().state())
        .isEqualTo(TeleportDestinationState.UNAVAILABLE);
  }
  ...
  private static WorldSnapshot world(String name, boolean loaded) {
    return new WorldSnapshot(
        name, name, name, loaded, "NORMAL", 1L, "", "", "NORMAL", true, true, name + " 0 64 0");
  }
  ...
  private static final class FakeGateway implements WorldProvider {
    private List<WorldSnapshot> worlds;
    private int loadCalls;
    private int catalogCalls;
    private int worldLookupCalls;
    ...
    public Optional<WorldSnapshot> world(String name) {
      ...
      return worlds.stream()
          .filter(
              world ->
                  world.name().equalsIgnoreCase(name) || world.identity().equalsIgnoreCase(name))
          .findFirst();
    }
