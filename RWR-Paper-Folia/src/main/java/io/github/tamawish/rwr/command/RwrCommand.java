...
public final class RwrCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<CommandHelp> COMMANDS = List.of(
            ...
            new CommandHelp("discord", "/rwr discord <reload|status>", "Manage the Discord webhook add-on", "rwrdiscord.admin", false),
            new CommandHelp("placeholder", "/rwr placeholder <reload|status>", "Manage the PlaceholderAPI add-on", "rwrplaceholder.admin", false));
    ...
    private void help(CommandSender sender) {
        ...
        COMMANDS.stream().filter(entry -> visibleTo(sender, entry) && available(entry)).forEach(entry -> messages.send(
    ...
    private void sendResetOutcome(CommandSender sender, ResetOutcome outcome) {
        if (outcome.successful()) {
            ...
            messages.send(
                    ...
                    "failure",
                    displayFailure(outcome.failure()),
                    "safety",
                    outcome.safety(),
                    "operation",
                    outcome.operationId(),
    ...
    private void history(CommandSender sender, String value) {
        ...
        entries.forEach(entry -> {
            ...
            messages.send(sender, "command.history-detail",
                    "result",
                    entry.failure() == null ? "Successful" : displayFailure(entry.failure()) + " / " + entry.safety(),
                    "message", paperTerminology(entry.message()));
        });
    }
    ...
    private void sendStatus(CommandSender sender, ResetStatus status) {
        ...
        messages.send(sender, "command.status-operation", "operation", shortOperation(status.operationId()),
                "message", paperTerminology(status.message()));
    }
    ...
    private static String displayFailure(Object failure) {
        return failure == null ? "none" : String.valueOf(failure).replace("MULTIVERSE", "WORLDS");
    }
