...
/**
 * Platform port for world catalog, safe destinations, and regeneration. Implemented by Multiverse
 * (Spigot) and Worlds (Paper-Folia).
 */
public interface WorldProvider extends WorldCatalogView {
  ...
  default String canonicalWorldName(String name) {
    return world(name).map(WorldSnapshot::identity).orElse(name);
  }
